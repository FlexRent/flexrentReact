...
export default function Login() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const loginResponse = await fetch("https://flexrent-fake-api2.vercel.app/api/login", {
        method: "POST",
        headers: {
      ...
      if (loginData.token !== undefined) {
        const userResponse = await fetch("https://flexrent-fake-api2.vercel.app/api/user", {
          headers: {
